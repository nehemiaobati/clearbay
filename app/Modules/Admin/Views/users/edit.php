...
<div class="container py-5 mt-5">
  ...
  <div class="mb-4 reveal">
    <a href="<?= url_to('admin.users.list') ?>" class="mono-label text-decoration-none" style="color: var(--color-brand-primary);">← Back to Users</a>
  </div>
  ...
  <div class="row justify-content-start">
    <div class="col-lg-8">
      <div class="card border-secondary border-opacity-10 p-4 p-md-5 reveal" style="background: var(--color-bg-card);">
        ...
        <form action="<?= $isEdit ? url_to('admin.users.update', $user->id) : url_to('admin.users.create') ?>" method="POST" novalidate>
          <?= csrf_field() ?>

          <!-- Name -->
          <div class="mb-3">
            <div class="form-floating">
              <input type="text" name="name" id="nameInput" class="form-control bg-dark bg-opacity-25 border-secondary border-opacity-25 <?= session('errors.name') ? 'is-invalid' : '' ?>" placeholder="Full Name"
          ...
          <div class="mb-3">
            <div class="form-floating">
              <input type="email" name="email" id="emailInput" class="form-control bg-dark bg-opacity-25 border-secondary border-opacity-25 <?= session('errors.email') ? 'is-invalid' : '' ?>" placeholder="Email Address"
          ...
          <div class="mb-3">
            <div class="form-floating">
              <select name="role" id="roleSelect" class="form-select bg-dark bg-opacity-25 border-secondary border-opacity-25 <?= session('errors.role') ? 'is-invalid' : '' ?>" required>
          ...
          <div class="mb-3 d-none" id="hospitalGroup">
            <div class="form-floating">
              <select name="hospital_id" id="hospitalSelect" class="form-select bg-dark bg-opacity-25 border-secondary border-opacity-25">
          ...
          <div class="mb-3 d-none" id="emsGroup">
            <div class="form-floating">
              <select name="ems_provider_id" id="emsSelect" class="form-select bg-dark bg-opacity-25 border-secondary border-opacity-25">
                <option value="" selected>Unassigned / None</option>
          ...
          <div class="mb-3 d-none" id="ambulanceGroup">
            <div class="form-floating">
              <select name="ambulance_id" id="ambulanceSelect" class="form-select bg-dark bg-opacity-25 border-secondary border-opacity-25">
                <option value="" selected>Unassigned / None</option>
            ...
            <div class="small text-secondary mt-1">Each ambulance can only be assigned to one active paramedic at a time.</div>
          </div>

          <!-- Active Status -->
          <div class="mb-3">
            <div class="form-floating">
              <select name="active" id="activeSelect" class="form-select bg-dark bg-opacity-25 border-secondary border-opacity-25" required>
          ...
          <?php if ($isEdit) : ?>
            <details class="mb-3" open>
              <summary class="mono-label text-secondary border-bottom pb-2 mb-3" style="cursor: pointer;">Set New Password (Optional)</summary>
              <div class="mb-3">
                <div class="input-group">
                  <div class="form-floating flex-grow-1">
                    <input type="password" name="new_password" id="newPasswordInput" class="form-control bg-dark bg-opacity-25 border-secondary border-opacity-25 <?= session('errors.new_password') ? 'is-invalid' : '' ?>" placeholder="Password" autocomplete="new-password">
                    <label for="newPasswordInput">New Password</label>
                  </div>
                  <button type="button" class="btn btn-outline-secondary touch-target" id="generatePassBtn" title="Generate secure password">🔑</button>
                  <button type="button" class="btn btn-outline-secondary touch-target" id="togglePassBtn" title="Show/Hide password">👁️</button>
                </div>
                <?php if (session('errors.new_password')) : ?>
                  <div class="text-danger small mt-1"><?= (string) esc(session('errors.new_password')) ?></div>
                <?php endif; ?>
                <div class="small text-secondary mt-1">Min 6 characters. Leave blank to keep the current password unchanged.</div>
              </div>
            </details>
          ...
          <div class="d-flex align-items-center gap-3 mt-4">
            <button type="submit" class="btn btn-primary flex-fill touch-target">
              <?= $isEdit ? 'Save Changes' : 'Register Account' ?>
            </button>
            <a href="<?= url_to('admin.users.list') ?>" class="btn btn-outline-secondary touch-target">
              Cancel
            </a>
          </div>
        </form>
